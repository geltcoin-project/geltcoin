...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 25034
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 25035
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 125034
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 125035
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 25034
	%{_sbindir}/semanage port -d -p tcp 25035
	%{_sbindir}/semanage port -d -p tcp 125034
	%{_sbindir}/semanage port -d -p tcp 125035
	for selinuxvariant in %{selinux_variants}; do
