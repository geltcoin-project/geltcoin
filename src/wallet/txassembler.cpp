    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional GELTC outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes GELTC and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::GELTC_TO_GELTC) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_geltc);
        }
    } else {
        // First try to construct a GELTC-to-GELTC transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::GELTC_ONLY;
        ...
        // If GELTC-to-GELTC fails, create a peg-out transaction
