Geltcoin Core version *0.15.1* is now available from:

  <https://download.geltcoin.org/geltcoin-0.15.1/>
...
A number of changes to the way Geltcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 geltites per kB (0.00001 GELTC). This is to prepare for dropping the minimum transaction fee to 0.1 geltites per kB (0.0001 GELTC) in the next release.
